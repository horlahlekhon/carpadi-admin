...
function UnderInspectionPage() {
    ...
    const retrieveCarList = (page = 0) => {
        setPageLoading(true)
        retrieveCars(rowsPerPage, page, [CarStates.ONGOING_INSPECTION, CarStates.PENDING_INSPECTION, CarStates.INSPECTED])
            .then((response) => {
                if (response.status) {
                    setPagination({
                                    ...
                                    {cars
                                        .map((row, idx) => (
                                            ...
                                            >
                                                ...
                                                <TableCell
                                                    align="left">{formatDate(row?.information?.created)}</TableCell>
                                                <TableCell align="left">
                                                    <Button
                                                        text="View"
                                                        width={66}
                                                        outlined={true}
                                                        onClick={() => handleNavigation(`/inventory/car-profile/${row.id}?status=under inspection`)}
                                                    />
                                                </TableCell>
                                            </TableRow>
                                        ))}
...
export default UnderInspectionPage

UnderInspectionPage.getLayout = function getLayout(page) {
