...
const retrieveMerchants = (limit = 10, offset = 1, tradeStatus = "", user = "") => {
    return fetchWrapper.get(`${baseUrl}/merchants?limit=${limit}&offset=${offset}&trading_status=${tradeStatus}&user=${user}`)
        .then((response) => {
...
const updateSingleMerchant = (id, data) => {
    return fetchWrapper.put(`${baseUrl}/merchants/${id}/`, data)
        .then((response) => {
            return {status: true, data: response}
        })
        .catch((error) => {
